...
def is_license_document(text: str) -> bool:
    ...
    if "DRIVER" in upper and "LICEN" in upper:
        return True
    if "DRIVING" in upper and "LICEN" in upper:
        return True
    if "DMV" in upper:
        return True
...
def extract_license_number(text: str) -> str | None:
    ...
    patterns = (
        ...
        r"\b([A-Z]{2}[0-9O]{2}[\s\-]?[0-9O]{4,15})\b",  # Indian driving license (e.g., MH10 20250008848)
        r"\b([A-Z]{2}\d{6,10})\b",
    ...
    for pattern in patterns:
        ...
        if match:
            # Normalize possible O/0 confusions in the digit parts for Indian DLs
            extracted = re.sub(r"[\s\-]", "", match.group(1))
            if extracted[:2].isalpha() and len(extracted) >= 10:
                extracted = extracted[:2] + extracted[2:].replace("O", "0")
            return extracted
    return None
